...
proc newBrushEngine*(): NBrushEngine =
  ...
  block: # -- Define Textures and Framebuffers
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, cast[GLint](GL_RED), 
      2048, 2048, 0, GL_R8, GL_UNSIGNED_BYTE, nil)
    # Set Mig/Mag Filter
    glTexParameteri(GL_TEXTURE_2D, 
      GL_TEXTURE_MIN_FILTER, cast[GLint](GL_NEAREST))
    glTexParameteri(GL_TEXTURE_2D, 
...
proc test*(brush: var NBrushEngine) =
...
proc begin*(brush: var NBrushEngine) =
  # Backup GUI Viewport
...
# --> Debug Prototype
proc clear*(brush: var NBrushEngine) =
  glClearColor(1.0, 1.0, 1.0, 1.0)
  glClear(GL_COLOR_BUFFER_BIT)

proc transform*(brush: var NBrushEngine, x, y, s, o: float32) =
...
proc quad*(brush: var NBrushEngine): NQuad =
  for i in 0..3: # Iterate Points
    result[i] = brush.quad[i]
    vec2_mat3(result[i], brush.matrix)

proc draw*(brush: var NBrushEngine) {.inline.} =
  glDrawArrays(GL_TRIANGLE_STRIP, 0, 4)

proc finish*(brush: var NBrushEngine) =
